...
class McuDevice:
    ...
    def __init__(self, isExtender: bool):
        ...
        # create tracks
        self._tracks = [mcu_device_track.McuDeviceTrack(
            i, self.__productId, i == 8) for i in range(8 if isExtender else 9)]
    ...
    def Initialize(self):
        ...
        if device.isAssigned():
            device.midiOutSysex(
                bytes([0xF0, 0x00, 0x00, 0x66, self.__productId, 0x0C, 1, 0xF7]))
    ...
    def SetBackLightTimeout(self, Minutes):
        ...
        if device.isAssigned():
            device.midiOutSysex(
                bytes([0xF0, 0x00, 0x00, 0x66, self.__productId, 0x0B, Minutes, 0xF7]))
    ...
    def SetClicking(self, enabled: bool):
        ...
        if device.isAssigned():
            device.midiOutSysex(
                bytes([0xF0, 0x00, 0x00, 0x66, self.__productId, 0x0A, int(enabled), 0xF7]))
    ...
    def EnableMeters(self):
        ...
        if device.isAssigned():
            ...
            device.midiOutSysex(
                bytes([0xF0, 0x00, 0x00, 0x66, self.__productId, 0x21, 1, 0xF7]))
            # enable meters
    ...
    def SetTextDisplay(self, message, row: int = 0, skipIsAssignedCheck: bool = False):
        ...
        if skipIsAssignedCheck or device.isAssigned():
            ...
            sysex = bytearray([0xF0, 0x00, 0x00, 0x66, self.__productId, 0x12, (lastMsgLen + 1)
                              * row]) + bytearray(message.ljust(lastMsgLen + 1, ' ')[:maxLen], 'utf-8')
            sysex.append(0xF7)
    ...
    def SetAssignmentMessage(self, number=-1, skipIsAssignedCheck: bool = False):
        ...
        if skipIsAssignedCheck or device.isAssigned():
            device.midiOutMsg(midi.MIDI_CONTROLCHANGE +
                              ((0x4B) << 8) + (ord(message[0]) << 16))
            device.midiOutMsg(midi.MIDI_CONTROLCHANGE +
                              ((0x4A) << 8) + (ord(message[1]) << 16))
