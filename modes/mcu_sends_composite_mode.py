...
class McuSendsCompositeMode(McuCompositeMode):
    """
    Sends mode: the regular sends overview, plus the Flip menu as a second sub-mode.
    """

    def __init__(self, device: McuDevice, trackBankingManager: TrackBankingManager, menuBankingManager: TrackBankingManager):
        super().__init__(device, [ModeButtonsBehavior(device, mcu_buttons.Sends)])

        overviewMode = McuSendsMode(device, trackBankingManager, self)
